    ...
    <section>
      ...
      {countryData &&
        ...
        (!countryData.country.length ? (
          ...
          <article className="grid grid-cols-2 sm:grid-cols-3 gap-4 my-4">
            {countryData &&
              countryData.country
                .filter((country) =>
                ...
                .map((country, index) => (
                  <div
                    key={index}
                    className="shadow-lg bg-services p-1 bg-white "
                  >
                    <div className="flex items-center gap-2">
                      <img
                        src={`https://flagcdn.com/w40/${country.iso2.toLowerCase()}.png`}
                        alt={country.name}
                        className="w-full object-cover h-32 rounded-sm"
                      />
                    </div>
                    ...
                    <div className="flex justify-between py-2 pr-2">
                      <button
                        className=" bg-visit py-1 px-5 rounded-lg font-bold"
                        onClick={(e) => {
                          const parentElement =
                      ...
                      >
                        <FaHeart
                          onClick={(e) => {
                            ...
                            const parentElement =
                              target.closest("div").parentElement.children;
                            const flag =
                              parentElement[0].querySelector("img").src;
                            const name = parentElement[1].textContent;
