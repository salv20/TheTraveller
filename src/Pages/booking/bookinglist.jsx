...
const Bookinglist = ({ storageFunc, fetchStorage }) => {
  ...
  return (
    <article className="capitalize font-semibold space-y-5 pt-10">
      <h1 className="font-bold text-lg">all bookings</h1>
      {Details.length ? (
        Details.map((country) => (
          <div
            className="bookContainer flex bg-heading rounded-r-full p-3 py-8"
            key={country.id}
            id={country.id}
          >
            <div className="w-full space-y-3">
              ...
              <div className="grid grid-cols-3">
                <p>{country.airline}</p>
                <p>{`$${Math.round(
                  Number(country.fee) * Number(country.passenger)
                )}.00`}</p>
                <p>{country.class}</p>
              </div>
            </div>
        ...
        <div className="bg-heading border rounded-mdfont-bold p-5 text-center mt-60 mx-auto w-3/4">
